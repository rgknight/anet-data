...
src.loc = "C:/Dropbox (UP)/UP-Data Evaluation/Sacred Data/ANet/FY15/School Results"
...
for(i in files) {
  ...
  # read
  X <- read.csv(paste("Data",i, sep="/"), header=TRUE)
  ...
  # Reshape to long
  X <- X %>% select(-State) 
  long <- X %>% gather(measure, value, 3:ncol(X))
...
f <- f %>% 
  ...
  rename(Anet.School.Id = School.Id) %>%
...
write.csv(f, file ="FY15 Anet All Schools All Grades.csv", row.names = F)
