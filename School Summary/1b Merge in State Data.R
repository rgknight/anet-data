...
f <- read.csv("C:/Dropbox (UP)/UP-Data Evaluation/Sacred Data/ANet/FY15/School Results/FY15 Anet All Schools All Grades.csv")

# Merge in codes from anet
anet.names <- read.csv(paste(sacred.source,"ANet Name Matching.csv", sep="/"))
anet.names <- anet.names %>%
  filter(!is.na(Anet.Name) & !duplicated(Anet.School.Id) ) %>%
  select( -Anet.Name, -Notes)

f <- left_join(f, anet.names, by = c("Anet.School.Id" = "Anet.School.Id" ))
...
# Merge MCAS and ANET
anet.merged <- left_join(f, m, by = c("subject", "grade", "State.ID"))


write.csv(anet.merged, file=paste(out.loc, "Tables/ANet and MCAS merged.csv", sep = "/"),
          row.names=F, na="")


# Append MCAS instead of mergisng
...
write.csv(anet.append, file=paste(out.loc, "Tables/ANet and MCAS appended.csv", sep = "/"),
          row.names=F, na="")
