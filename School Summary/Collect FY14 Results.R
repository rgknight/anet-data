...
src.loc = "C:/Dropbox (UP)/UP-Data Evaluation/Sacred Data/ANet/FY14/School Results"
...
for(i in files) {
  ...
  # Reshape to long
  X <- X %>% select(-State) 
  long <- X %>% gather(measure, value, 2:ncol(X))
...
write.csv(f, file ="FY14 Anet All Schools All Grades.csv", row.names = F)
